...
if (strtoupper((string) ($_SERVER['REQUEST_METHOD'] ?? 'GET')) === 'POST') {
    try {
        ...
        if ($action === 'save') {
            $connectionType = strtolower(trim((string) ($_POST['connection_type'] ?? 'file')));
            if (!in_array($connectionType, ['file', 'network', 'browser'], true)) {
            ...
            $config = $routing->buildPrinterConfig([
                ...
                'simulator_key' => $_POST['simulator_key'] ?? '',
                'host' => $_POST['host'] ?? '',
        ...
        $error = $exception->getMessage() !== '' ? $exception->getMessage() : 'PRINT_ACTION_FAILED';
    }
}
...
try {
    ...
    $error = $error ?: $loadError->getMessage();
}
    ...
    <div class="container-fluid">
      <header class="printer-hero">
        ...
        <div class="printer-mode <?= $mode === 'silent' ? 'is-live' : '' ?>">
          ...
          <small>يُغيَّر من إعداد <code>POSMAIN_PRINT_MODE</code> فقط.</small>
        </div>
      </header>
      ...
      <div class="printer-layout">
            ...
            <div class="printer-two">
              ...
              <label>طريقة الاتصال
                <select name="connection_type" id="printerConnection">
                  <option value="file">محاكي محلي</option>
                  <option value="network">شبكة مباشرة</option>
                  <option value="browser">متصفح (قديم فقط)</option>
                </select>
              </label>
            </div>

            <div class="printer-transport is-visible" data-transport="file">
              <label>مفتاح المحاكي
                <input type="text" name="simulator_key" id="printerSimulatorKey" maxlength="64" placeholder="kitchen-drinks">
                <small>اسم آمن لمجلد الاختبار؛ لا يقبل مسار ملفات.</small>
              </label>
            </div>
            <div class="printer-transport printer-two" data-transport="network">
              <label>عنوان الطابعة
        ...
        <section class="printer-panel">
          ...
          <div class="printer-list">
            <?php if (!$printers): ?>
              <div class="printer-empty"><i class="fas fa-print"></i><h3>لا توجد طابعات بعد</h3><p>ابدأ بمحاكي الإيصالات ومحاكي لكل محطة مطبخ.</p></div>
            ...
            ?>
              <article class="printer-card <?= $printer['is_active'] ? '' : 'is-disabled' ?>">
                <div class="printer-card-top">
                  <div class="printer-icon"><i class="fas fa-print"></i></div>
                  <div><h3><?= printerManagementH($printer['name']) ?></h3><p><?= printerManagementH($printer['connection_type']) ?> · <?= printerManagementH($printer['printer_type']) ?></p></div>
                  <span class="printer-status"><?= $printer['is_active'] ? 'نشطة' : 'معطلة' ?></span>
                </div>
          ...
          <table>
            ...
            <tbody>
              ...
              <?php else: foreach ($recentJobs as $job): ?>
                <tr>
                  <td>#<?= $job['id'] ?></td>
                  <td><?= printerManagementH($job['job_type']) ?></td>
                  <td><span class="job-status is-<?= printerManagementH($job['status']) ?>"><?= printerManagementH($job['status']) ?></span><?php if ($job['last_error']): ?><small><?= printerManagementH($job['last_error']) ?></small><?php endif; ?></td>
                  <td><?= $job['attempts'] ?> / <?= $job['max_attempts'] ?></td>
                  <td><?= printerManagementH($job['created_at']) ?></td>
                  <td><?php if ($job['status'] === 'failed'):
                    $uncertainDelivery = str_starts_with((string) ($job['last_error'] ?? ''), 'PRINT_NETWORK_DELIVERY_UNCERTAIN');
...
(function () {
  ...
  document.addEventListener('click', function (event) {
    ...
    connection.value = printer.connection_type || 'file';
    document.getElementById('printerSimulatorKey').value = config.simulator_key || '';
