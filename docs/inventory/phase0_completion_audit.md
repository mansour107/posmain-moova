...
| Verify quantity read/monitoring surfaces | `current_schema_findings.md` maps current item amount reads across item list, opening balance, invoice item popup, item summary, stock levels, adjustments, inventory reports, recipe operational dashboard, and recipe availability; the current main dashboard has no stock-bearing widget. | Source-level proven |
