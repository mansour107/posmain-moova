...
| Main dashboard | No stock quantity is rendered by the current attention/sales-strip component | No inventory source is consulted | Re-run the stock-read contract if a stock-bearing dashboard widget is reintroduced | `elements/main/main_tables.php` |
