...
try {
    ...
    // Generic bank is an authenticated manual settlement: it posts now,
    // records who declared it, needs no external reference, and never touches cash.
    $conn->query("INSERT INTO ot_head (id, pro_tybe, fat_net, payment_status) VALUES (708, 9, 11.00, 'paid')");
    $conn->query("
        INSERT INTO fat_details (id, fatid, item_id, qty_in, qty_out, price, discount, det_value, posted_qty, posted_unit_price, posted_net, posted_tax, tax_rate_snapshot)
        VALUES (10, 708, 19, 0, 1, 11.000000, 0, 11.00, 1.000000, 11.000000, 11.00, 0.00, 0.000000)
    ");
    $conn->query("INSERT INTO order_payments (id, order_id, amount, payment_method) VALUES (99, 708, 11.00, 'cash')");
    $selectedBank = $service->createPostedRefund($conn, [
        'original_order_id' => 708,
        'customer_account_id' => 501,
        'revenue_account_id' => 91,
        'user_id' => 8,
        'reason' => 'Authenticated manual bank payout',
        'idempotency_key' => 'refund-708-selected-bank',
        'refund_payment_method' => 'bank',
    ], ['sync_config' => $syncConfig]);
    $bankTender = $selectedBank['refund_tenders'][0] ?? [];
    financialRefundAssert($selectedBank['pending_external_amount'] === '0.00', 'manual bank must not create pending external liability');
    financialRefundAssert(($bankTender['status'] ?? '') === 'settled', 'manual bank must settle immediately');
    financialRefundAssert(($bankTender['settlement_policy'] ?? '') === 'manual_external', 'response must expose manual bank policy');
    financialRefundAssert((int) ($bankTender['settlement_declared_by'] ?? 0) === 8, 'manual bank must persist declaring operator');
    financialRefundAssert(!empty($bankTender['settlement_declared_at']), 'manual bank must persist declaration time');
    financialRefundAssert((int) ($bankTender['journal_head_id'] ?? 0) > 0, 'manual bank must post a settlement journal');
    financialRefundAssert(
        (int) $conn->query("SELECT COUNT(*) AS c FROM drawer_movements WHERE order_id = 708")->fetch_assoc()['c'] === 0,
        'manual bank must never create drawer movement'
    );
...
function financialRefundCreateLegacySchema(mysqli $conn): void
{
    ...
    ");
    $conn->query("INSERT INTO acc_head (id, code, name, balance) VALUES (51, '51', 'cash', 0), (52, '52', 'card', 0), (53, '53', 'vat', 0), (54, '54', 'bank', 0), (501, '501', 'ar', 0), (91, '91', 'sales', 0)");
    $conn->query("
