...
try {
    ...
    paidReversalAssert(
        $recipeSpy->refunded[1]['order']['lines'][0]['stock_disposition'] === 'restock',
        'partial recipe reversal must carry the immutable posted credit-note stock disposition'
    );
    paidReversalAssert(
        (int) $recipeSpy->refunded[1]['order']['lines'][0]['credit_note_line_id'] > 0,
        'partial recipe reversal must carry its durable credit-note-line identity'
    );
