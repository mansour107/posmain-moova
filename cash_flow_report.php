...
$orders = $canSalesReports && $context['tab'] === $tabOrders
    ...
    : [];
$refundRows = $canSalesReports && $context['tab'] === $tabPayments
    ? $operationsReports->refunds($conn, $periodFilters)
    : [];
...
$movementLabels = [
    ...
    'closing_adjustment' => 'تعديل رصيد الإغلاق', 'no_sale' => 'فتح الدرج دون بيع',
];
$statusLabels = ['open' => 'مفتوح', 'closed' => 'مغلق', 'forced_closed' => 'أغلقته الإدارة'];
$orderStatusLabels = [
    'paid' => 'مدفوع', 'refunded' => 'مسترد', 'unpaid' => 'غير مدفوع',
    'cancelled' => 'ملغى', 'canceled' => 'ملغى', 'voided' => 'ملغى',
...
$paymentSourceLabels = [
    'payment' => 'من عمليات الدفع المسجلة',
    'drawer' => 'من سجلات الدفع والدرج',
    'legacy' => 'من عمليات الدفع المسجلة',
    'fallback' => 'من بيانات الطلبات',
    'none' => 'لا توجد بيانات',
];
$varianceTypeLabels = ['opening' => 'رصيد البداية', 'closing' => 'رصيد الإغلاق', 'both' => 'البداية والإغلاق', 'force_close' => 'إغلاق بواسطة الإدارة', 'none' => '—'];
...
$attentionDescriptions = [
    ...
    'pending_refunds' => 'عرض المرتجعات التي لم يصل مبلغها للعميل بعد',
    ...
    <main class="premium-report cash-shift-workspace" id="cashShiftWorkspace" dir="rtl" lang="ar">
      ...
      <section class="pr-panel pr-no-print cash-shift-filter-panel">
        ...
        <form method="get" id="cashFlowFilters" class="pr-filters-grid">
          ...
          <?php if ($context['tab'] === $tabShifts): ?><div class="pr-field"><label for="override_operator_id">مستخدم شارك في الشيفت</label><select id="override_operator_id" name="override_operator_id" class="form-control"><option value="0">جميع المستخدمين</option><?php foreach ($cashiers as $cashier): ?><option value="<?= (int) $cashier['id'] ?>" <?= $context['override_operator_id'] === (int) $cashier['id'] ? 'selected' : '' ?>><?= htmlspecialchars((string) ($cashier['display_name'] ?: $cashier['uname'])) ?></option><?php endforeach; ?></select></div><?php endif; ?>
      ...
      <section class="operations-intro" aria-labelledby="salesSummaryHeading">
        <div>
          ...
          <p>يعرض هذا الملخص قيمة الأصناف، ثم يوضح الخصومات والإضافات والضرائب والمرتجعات للوصول إلى الصافي.</p>
        </div>
        <span class="pr-pill pr-pill--status-closed">حسب يوم العمل</span>
      </section>
      <div class="operations-metrics">
        <article class="operations-metric operations-metric--primary"><span>صافي المبيعات</span><strong><?= number_format((float)($salesSummary['net_sales'] ?? 0), 2) ?></strong><small>قيمة الأصناف − الخصومات + الإضافات + الضرائب − المرتجعات</small></article>
        <article class="operations-metric"><span>قيمة الأصناف</span><strong><?= number_format((float)($salesSummary['gross_sales'] ?? 0), 2) ?></strong><small>قبل الخصومات والإضافات والضرائب والمرتجعات</small></article>
        ...
        <article class="operations-metric"><span>الخدمة والإضافات</span><strong>+ <?= number_format((float)($salesSummary['service_plus'] ?? 0), 2) ?></strong><small>إضافات محسوبة ضمن صافي الطلبات</small></article>
        <article class="operations-metric"><span>الضرائب</span><strong>+ <?= number_format((float)($salesSummary['tax'] ?? 0), 2) ?></strong><small>ضرائب محسوبة ضمن صافي الطلبات</small></article>
        ...
        <article class="operations-metric"><span>عدد الطلبات</span><strong><?= number_format((int)($salesSummary['order_count'] ?? 0)) ?></strong><small>الطلبات المدفوعة، ومنها الطلبات التي تم استردادها</small></article>
      ...
      <?php if ($canReport): ?><section class="pr-panel cash-walk-panel">
        <div class="pr-panel-head"><div><p class="pr-eyebrow">النقدية في الأدراج</p><h2 class="pr-panel-title">حركة النقدية</h2></div><a class="pr-btn pr-btn-ghost" href="<?= htmlspecialchars($tabUrl($tabMovements), ENT_QUOTES, 'UTF-8') ?>">فتح سجل النقدية</a></div>
        <div class="pr-panel-body cash-walk-grid">
          ...
          <?php endforeach; ?>
          <div class="cash-walk-step is-result"><span>المبلغ المتوقع وجوده في الأدراج</span><strong><?= number_format($expectedRollup, 2) ?></strong></div>
        </div>
      </section><?php endif; ?>
      ...
      <section class="pr-panel">
        <div class="pr-panel-head"><div><p class="pr-eyebrow"><?= $context['scope'] === 'backlog' ? 'كل الفترات' : 'الفترة المختارة' ?></p><h2 class="pr-panel-title">الشيفتات وحركة كل درج</h2></div><span class="pr-pill pr-pill--muted"><?= (int) $shiftsPage['total'] ?> شيفت</span></div>
        <div class="pr-panel-body">
          <div class="pr-session-toolbar"><div class="pr-chip-filters" role="group" aria-label="تصفية حالة الشيفت">
            <?php foreach (['all'=>'الكل','open'=>'مفتوح','closed'=>'مغلق','needs_review'=>'تحتاج مراجعة','forced_closed'=>'أغلقته الإدارة'] as $key=>$label): ?>
            ...
            <?php endforeach; ?>
            <a href="<?= htmlspecialchars($workspaceUrl(['has_override'=>$context['has_override']?null:1,'page'=>null]), ENT_QUOTES, 'UTF-8') ?>" class="pr-chip-filter <?= $context['has_override'] ? 'is-active' : '' ?>"><i class="fas fa-user-clock"></i> شارك فيه مستخدم آخر</a>
          </div><div class="pr-session-meta">صفحة <?= (int) $shiftsPage['page'] ?> من <?= (int) $shiftsPage['pages'] ?></div></div>
          <p class="operations-source-note"><i class="fas fa-database"></i> الأرقام مأخوذة مباشرة من حركات الدرج في كل شيفت. ويظهر داخل النقدية الخارجة تفصيل المرتجعات والمصروفات والتوريد إلى الخزنة.</p>
          <div class="pr-table-wrap pr-table-scroll"><table class="pr-table pr-table--compact cash-shift-table"><thead><tr><th>الكاشير</th><th>يوم العمل / الوقت</th><th>رصيد البداية</th><th>المبيعات النقدية</th><th>نقدية أخرى دخلت الدرج</th><th>نقدية خرجت من الدرج</th><th>المبلغ المتوقع في الدرج</th><th>الفرق</th><th>الحالة</th><th>الإجراءات</th></tr></thead><tbody>
          <?php foreach ($shiftsPage['rows'] as $session):
              ...
              $needsReview = in_array(
                  (string) ($session['variance_status'] ?? ''),
                  ['counted_pending_review', 'unresolved'],
                  true
              );
          ...
          <tr class="<?= $needsReview ? 'pr-row-variance' : '' ?>">
            <td data-testid="cash-shift-accountability"><span class="pr-pill pr-pill--user"><?= htmlspecialchars($displayPerson((string) ($session['user_name'] ?? ''))) ?></span><?php if (!empty($session['has_override'])): ?> <span class="pr-pill pr-pill--warn">شارك مستخدم آخر</span><?php endif; ?><small class="operations-cell-sub">شيفت #<?= (int)$session['id'] ?> · أغلقه <?= htmlspecialchars($displayPerson((string)($session['closed_by_name'] ?? '') ?: '—')) ?></small></td>
      ...
      <section class="pr-panel">
        ...
        <div class="pr-panel-body">
          ...
          <p class="operations-source-note"><i class="fas fa-info-circle"></i> يعرض هذا الجدول جميع الطلبات وحالتها الحالية. ملخص المبيعات يحسب الطلبات المدفوعة ويخصم منها المرتجعات.</p>
          ...
          <?php foreach ($orders as $order):
              ...
              $reversalStatus = strtolower((string) ($order['reversal_status'] ?? 'none'));
              $isFullRefund = $reversalStatus === 'full';
              $isPartialRefund = $reversalStatus === 'partial';
          ...
          ?><tr class="<?= (int)$order['isdeleted'] === 1 ? 'pr-row-warn' : '' ?>">
            <td><a href="print/receipt.php?id=<?= (int)$order['id'] ?>" target="_blank" rel="noopener">#<?= htmlspecialchars((string)$order['public_order_number']) ?></a><small class="operations-cell-sub">رقم داخلي: <?= (int)$order['id'] ?></small></td>
            ...
            <td><span class="pr-pill <?= $statusClass ?>"><?= htmlspecialchars($isFullRefund ? 'مسترد بالكامل' : ($isPartialRefund ? 'مسترد جزئياً' : ($orderStatusLabels[strtolower($orderStatus)] ?? $orderStatus))) ?></span><?php if ($isFullRefund || $isPartialRefund): ?><small class="operations-cell-sub"><?= $isPartialRefund ? 'المسترد حتى الآن' : 'المبلغ المسترد' ?> <?= number_format((float)($order['cumulative_refunded_amount'] ?? 0), 2) ?><?php if ($isPartialRefund): ?> · المتبقي القابل للاسترداد <?= number_format((float)($order['remaining_refundable_amount'] ?? 0), 2) ?><?php endif; ?></small><?php endif; ?></td>
          </tr><?php endforeach; ?>
      ...
      <div class="operations-metrics operations-metrics--payments">
        ...
        <article class="operations-metric operations-metric--negative"><span>إجمالي مبالغ المرتجعات</span><strong>− <?= number_format((float)($payments['refund_total'] ?? 0),2) ?></strong><small>كل المرتجعات التي سجلها النظام، سواء اكتملت أو ما زالت قيد التنفيذ</small></article>
        <article class="operations-metric"><span>مرتجعات لم تصل للعميل بعد</span><strong><?= number_format((float)($payments['pending_external_refund_total'] ?? 0),2) ?></strong><small>مبالغ مرتجعات البطاقة أو المحفظة التي لم يتأكد وصولها للعميل</small></article>
      </div>
      ...
      <section class="pr-panel" data-testid="refund-reversal-drilldown"><div class="pr-panel-head"><div><p class="pr-eyebrow">سجل المرتجعات المحفوظ</p><h2 class="pr-panel-title">تفاصيل كل مرتجع والبيع الأصلي</h2></div><span class="pr-pill pr-pill--muted"><?= count($refundRows) ?> عملية</span></div><div class="pr-panel-body"><p class="operations-source-note"><i class="fas fa-info-circle"></i> يظهر كل مرتجع في يوم تنفيذه وباسم الموظف والشيفت. وتظل عملية البيع الأصلية محفوظة ولا تُحذف.</p><div class="pr-table-wrap pr-table-scroll"><table class="pr-table operations-table"><thead><tr><th>رقم المرتجع</th><th>الطلب الأصلي</th><th>يوم التنفيذ</th><th>نفّذه</th><th>الشيفت</th><th>مبلغ هذا المرتجع</th><th>إجمالي ما تم استرداده</th><th>حالة الاسترداد</th><th>حالة إعادة المبلغ</th><th>اعتماد المدير</th><th>السبب</th></tr></thead><tbody>
      <?php foreach ($refundRows as $refund): $isFullRefund = (string)$refund['reversal_status'] === 'full'; ?><tr><td>#<?= (int)$refund['credit_note_id'] ?></td><td><a href="print/receipt.php?id=<?= (int)$refund['original_order_id'] ?>" target="_blank" rel="noopener">#<?= htmlspecialchars((string)(($refund['receipt_number'] ?? '') ?: ($refund['public_order_number'] ?? '') ?: $refund['original_order_id'])) ?></a><small class="operations-cell-sub">قيمة الطلب <?= number_format((float)($refund['original_amount'] ?? 0),2) ?></small></td><td><?= htmlspecialchars((string)$refund['business_day']) ?></td><td><?= htmlspecialchars($displayPerson((string)$refund['operator_name'])) ?></td><td><?php if(!empty($refund['drawer_session_id'])): ?><a href="<?= htmlspecialchars(posmain_cash_shift_detail_url((int)$refund['drawer_session_id'],$returnTo), ENT_QUOTES, 'UTF-8') ?>">#<?= (int)$refund['drawer_session_id'] ?></a><?php else: ?>—<?php endif; ?></td><td class="operations-negative">− <?= number_format((float)$refund['total_amount'],2) ?></td><td><?= number_format((float)$refund['cumulative_refunded_amount'],2) ?></td><td><span class="pr-pill <?= $isFullRefund?'pr-pill--expense':'pr-pill--warn' ?>"><?= $isFullRefund?'مسترد بالكامل':'مسترد جزئياً' ?></span><small class="operations-cell-sub">المتبقي القابل للاسترداد <?= number_format((float)$refund['remaining_refundable_amount'],2) ?></small></td><td><?php if((string)$refund['settlement_status']==='pending_external'): ?><span class="pr-pill pr-pill--warn">لم يصل بعد</span><small class="operations-cell-sub"><?= number_format((float)$refund['pending_external_amount'],2) ?></small><?php else: ?><span class="pr-pill pr-pill--status-closed">اكتمل</span><?php endif; ?></td><td><?= !empty($refund['manager_approval_id'])?'#'.(int)$refund['manager_approval_id']:'—' ?></td><td><?= htmlspecialchars((string)(($refund['reason']??'')?:'—')) ?></td></tr><?php endforeach; ?>
      <?php if($refundRows===[]): ?><tr><td colspan="11" class="text-center text-muted py-4">لا توجد مرتجعات منشورة في هذه الفترة.</td></tr><?php endif; ?>
      </tbody></table></div></div></section>
      <?php endif; ?>
      ...
      <div class="cash-shift-settings-grid">
        ...
        <section class="pr-panel"><div class="pr-panel-head"><div><p class="pr-eyebrow">نقدية بداية الدرج</p><h2 class="pr-panel-title">الرصيد الأساسي لبداية الدرج</h2></div><?php if(!$needsBaselineInit): ?><span class="pr-pill pr-pill--status-closed">تم الإعداد</span><?php endif; ?></div><div class="pr-panel-body"><?php if($needsBaselineInit && $canSetBaseline): ?><p>حدّد النقدية المتوقعة قبل فتح أول شيفت في الفرع.</p><button class="pr-btn pr-btn-primary" type="button" data-bs-toggle="modal" data-bs-target="#openingBaselineModal">تحديد رصيد البداية</button><?php elseif($needsBaselineInit): ?><div class="pr-callout pr-callout--warn">يجب أن يضبط هذا الفرع مستخدم لديه صلاحية تحديد رصيد البداية.</div><?php else: ?><p class="text-muted mb-0">يحسب النظام النقدية المتوقعة من آخر شيفت مغلق وحركات الدرج التالية له.</p><?php endif; ?><?php if(abs((float)($openingExpectation['unassigned_net']??0))>0.0001): ?><div class="pr-callout pr-callout--warn mt-3">صافي النقدية غير المرتبطة: <?= number_format(abs((float)$openingExpectation['unassigned_net']),2) ?></div><?php endif; ?></div></section>
      </div>
      <?php endif; ?>
    </main>
  </section>
