...
require_once __DIR__ . '/../classes/Inventory/InventoryUnpaidSaleReclassificationService.php';
...
if (isset($options['help'])) {
    echo "Usage: php tools/inventory_reclassify_unpaid_sales.php [--plan|--rehearse|--apply --reviewed-manifest=<sha256> --backup=/absolute/backup.sql] [--json]\n";
    exit(0);
}
...
try {
    ...
    $service = new InventoryUnpaidSaleReclassificationService();
    $result = $mode === 'apply'
...
if (isset($options['json'])) {
    ...
    echo 'Unpaid sale reclassification: ' . (!empty($result['ok']) ? 'READY' : 'BLOCKED') . PHP_EOL;
