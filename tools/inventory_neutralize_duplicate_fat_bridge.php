...
require_once __DIR__ . '/../classes/Inventory/InventoryDuplicateFatBridgeNeutralizationService.php';
...
if (isset($options['help'])) {
    fwrite(STDOUT, "Usage: php tools/inventory_neutralize_duplicate_fat_bridge.php [--operational-store=27] [--plan|--rehearse|--apply --reviewed-manifest=<sha256> --backup=/absolute/backup.sql] [--json]\n");
    exit(0);
}
...
try {
    ...
    $service = new InventoryDuplicateFatBridgeNeutralizationService();
    if ($mode === 'apply') {
...
if (isset($options['json'])) {
    ...
    echo 'Duplicate legacy/canonical inventory neutralization: ' . (!empty($result['ok']) ? 'READY' : 'BLOCKED') . PHP_EOL;
