...
require_once __DIR__ . '/../classes/Inventory/InventoryStoreScopeReclassificationService.php';
...
if (isset($options['help'])) {
    fwrite(STDOUT, "Usage: php tools/inventory_reclassify_store_scope.php --source-stores=274 [--operational-store=27] [--plan|--rehearse|--apply --reviewed-manifest=<sha256> --backup=/absolute/backup.sql] [--json]\n");
    exit(0);
}
...
$mode = $modes[0] ?? 'plan';
$sourceStores = array_values(array_filter(array_map('intval', preg_split('/,/', (string) ($options['source-stores'] ?? '')) ?: []), static fn(int $id): bool => $id > 0));
$serviceOptions = [
    'source_store_ids' => $sourceStores,
];
...
try {
    ...
    $service = new InventoryStoreScopeReclassificationService();
    if ($mode === 'apply') {
...
if (isset($options['json'])) {
    ...
    echo 'Inventory store-scope reclassification: ' . (!empty($result['ok']) ? 'READY' : 'BLOCKED') . PHP_EOL;
    ...
    echo '- movements: ' . (int) ($result['summary']['movement_count'] ?? 0) . PHP_EOL;
