...
$canRead = auth_guard_has_permission('delivery.compensation.manage', $conn)
...
try {
    ...
    $service = new DeliveryCompensationService();
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        require_permission('delivery.compensation.manage', $conn);
        require_csrf('delivery_operations');
        $payload = $_POST;
        if (isset($payload['zone_rates_json'])) {
            $decoded = json_decode((string) $payload['zone_rates_json'], true);
            $payload['zone_rates'] = is_array($decoded) ? $decoded : [];
        }
        $plan = $service->savePlan($conn, $payload, ['user_id' => (int) ($_SESSION['userid'] ?? 0)] + $scope);
        echo json_encode(['success' => true, 'plan' => $plan], JSON_UNESCAPED_UNICODE);
        exit;
    }
    echo json_encode(['success' => true, 'plans' => $service->listPlans($conn, $scope, !empty($_GET['include_inactive']))], JSON_UNESCAPED_UNICODE);
} catch (Throwable $e) {
