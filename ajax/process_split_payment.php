...
pos_order_api_router_guard_direct_access('ajax/process_split_payment.php');
...
try {
    pos_api_emit_dispatch_result(pos_api_dispatch($conn, 'orders.split-payment'));
} catch (Throwable $e) {
    if (isset($conn) && $conn instanceof mysqli) {
        $conn->rollback();
    }
    $payload = pos_api_dispatch_exception_payload($e, 'orders.split-payment');
