...
$batchSize = isset($options['batch-size']) ? max(1, (int) $options['batch-size']) : 50;
...
if (!$once && !$loop) {
    fwrite(STDERR, "Usage: php cli/sync_worker.php --once|--loop [--batch-size=50] [--sleep=5] [--max-runtime=300]\n");
    exit(1);
}
